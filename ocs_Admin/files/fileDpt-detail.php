...
if (isset($_GET['detail']) && !empty($_GET['detail'])){
    ...
    $row = $student->getUserFiles('userRequestsDepartmentFinal',$requestid);
    if ($row){
        ...
        if ($user->data()->permission == 'student_hnd'){
            $all = array($row->school_form, $row->admission_letter,$row->acceptance_letter,$row->undertaken_form,$row->state_of_origin,$row->medical_report,$row->clearance_form_hod,$row->school_fees_breakdown,$row->olevel_result,$row->birth_certificate,$row->it_letter,$row->nd_result,$row->jamb_admission_letter,
                $row->nacoss_and_journal,$row->course_form,$row->bio_data_form, $row->final_clearance_form);
        ...
        Redirect::to('../../admin-requestDepartmentFinal');
    }
}
if (isset($_POST['approveClearance'])){
    $userid = $user->data()->id;
    $update = $student->updateRequest('userRequestsDepartmentFinal', $userid);
    if ($update){
...
if (isset($_POST['cancelapproveClearance'])){
    $userid = $user->data()->id;
    $update = $student->updateRequest2('userRequestsDepartmentFinal', $userid);
    if ($update){
            ...
            <div class="container shadow-lg" >
                ...
                <div class="row shadow-lg p-5">
                    <form action="#" method="post">
                        ...
                        <a href="<?=URLROOT?>ocs_Admin/admin-requestDepartmentFinal" class="btn btn-danger float-right">Return Back</a>
